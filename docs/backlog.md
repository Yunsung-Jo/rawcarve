...
|------|------|------|-----------|------------------------|
| W1 | 지표·분류 정비 | 기반 (저비용) | 이후 모든 실험의 평가 기준 확보 | 없으면 개선·악화 측정이 부정확(악화 66건이 장기 미발견된 재발 방지). 빈 huff 검증 포함 |
...
- **내용:** ① `report.csv`에 악화 플래그(`undec_after > undec_before`)·크기별 층화 통계, ② 무행동(ops 0·hole 1) 파일을 RECOVERED가 아닌 별도 action(예: FAILED)으로 분류(회색 재인코딩본이 복구본으로 저장되는 문제), ③ `Decoder` 빈 huff 검증(DHT 소실 파일이 SKIP이 아닌 RECOVERED·전량 회색으로 통과하는 분류 버그, `carver/jpegdecode.py:380-385`).
- **근거:** 악화 66건(평균 +0.354)·무행동 70건이 RECOVERED에 섞여 한 달 가까이 미발견 — [사각지대 조사](investigations/2026-07-02-recovery-tool-blindspots.md) 1단계·결론 4.
- **산출:** 전수 재실행으로 정확한 새 베이스라인 — W2~W6의 비교 기준.
